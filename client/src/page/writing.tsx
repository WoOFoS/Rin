        ...
        <div className="col-span-2 pb-8">
          <div className="bg-w rounded-2xl shadow-xl shadow-light p-4">
            ...
            <div className="flex flex-row items-center justify-end mb-4 space-x-2">
              <span className="text-sm text-neutral-500">{t('edit_mode') || '编辑模式'}:</span>
              <button
                onClick={() => setEditMode('markdown')}
                className={`px-3 py-1 rounded-lg text-sm ${editMode === 'markdown' ? 'bg-theme text-white' : 'bg-gray-200 dark:bg-gray-700 t-primary'}`}
              >
                Markdown
              </button>
              <button
                onClick={() => setEditMode('richtext')}
                className={`px-3 py-1 rounded-lg text-sm ${editMode === 'richtext' ? 'bg-theme text-white' : 'bg-gray-200 dark:bg-gray-700 t-primary'}`}
              >
                {t('rich_text') || '富文本'}
              </button>
            </div>
            {editMode === 'markdown' ? (
              ...
              <RichTextEditor content={content} setContent={setContent} height='600px' />
            )}
          </div>
